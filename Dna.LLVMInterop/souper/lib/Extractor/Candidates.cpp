...
Inst *ExprBuilder::makeArrayRead(Value *V) {
  ...
  if (true) {
    ...
    if (V->getType()->isIntegerTy()) {
      if (Instruction *I = dyn_cast<Instruction>(V)) {
        ...
        auto LVIRange = LVI->getConstantRange(V, I,/*isFullSet=*/true);
  ...
  llvm::outs() << "range from compiler: [" << Range.getLower() << ","
      << Range.getUpper() << ") " << Name.str().c_str() << " " << *V << "\n";
...
void ExprBuilder::addPathConditions(BlockPCs &BPCs,
  ...
  if (auto Pred = BB->getSinglePredecessor()) {
    ...
    if (Depth >= 25)
    {
        printf("hit recursion depth limit!\n");
    }
  } else if (ExploitBPCs) {
...
public:
 ExtractExprCandidatesPass(const ExprBuilderOptions &Opts, InstContext &IC,
  ...
  void getAnalysisUsage(AnalysisUsage &Info) const {
    ...
    Info.addRequired<DemandedBitsAnalysis>();
  ...
  bool runOnFunction(Function &F) {
    ...
    auto am = llvm::FunctionAnalysisManager();
    //DemandedBits* DB = getAnalysis<DemandedBitsAnalysis>().run(F, am);
